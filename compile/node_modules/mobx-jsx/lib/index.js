...
function memo(fn, equal) {
  const o = mobx.observable.box(mobx.untracked(fn)),
        update = mobx.action(r => o.set(r));
  effect(prev => {
    const res = fn();
...
function createComponent(Comp, props, dynamicKeys) {
  ...
  return mobx.untracked(() => Comp(props));
} // dynamic import to support code splitting

function lazy(fn) {
  return props => {
    let Comp;
    const result = mobx.observable.box(),
          update = mobx.action(component => result.set(component.default));
    fn().then(update);
    const rendered = mobx.computed(() => (Comp = result.get()) && mobx.untracked(() => Comp(props)));
    return () => rendered.get();
  };
} // context api
...
function resolveChildren(children) {
  if (typeof children === "function") {
    const c = mobx.observable.box(),
          update = mobx.action(child => c.set(child));
...
function createProvider(id) {
  return function provider(props) {
    let rendered = mobx.observable.box(),
        update = mobx.action(() => rendered.set(resolveChildren(props.children)));
    effect(() => {
      globalContext.context = {
        [id]: props.value
      };
...
function map(list, mapFn) {
  ...
  return () => {
    list[mobx.$mobx].atom.reportObserved();
    let newItems = list,
        i,
        j;
    return mobx.untracked(() => {
