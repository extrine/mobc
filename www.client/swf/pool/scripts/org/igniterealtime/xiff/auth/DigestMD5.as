package org.igniterealtime.xiff.auth
{
   ...
   public class DigestMD5 extends SASLAuth
   {
      ...
      {
         var md5:MD5 = new MD5();
         var password:String = this.connection.password;
         ...
         ha2 = md5.hash(ha2);
         var sha1:String = this.convertToBase16String(ha1);
         var sha2:String = this.convertToBase16String(ha2);
      ...
      override public function handleChallenge(stage:int, challenge:XML) : XML
      {
         ...
         if(!challengeMap.rspauth)
         {
            ...
            responseMap.username = this.connection.username;
            responseMap.realm = Boolean(challengeMap.realm)?challengeMap.realm:"";
            responseMap.nonce = challengeMap.nonce;
            responseMap.cnonce = new Date().time;
            responseMap.nc = "00000001";
            responseMap.qop = Boolean(challengeMap.qop)?challengeMap.qop:"auth";
            responseMap["digest-uri"] = "xmpp/" + (Boolean(challengeMap.realm)?challengeMap.realm:this.connection.domain);
            responseMap.charset = challengeMap.charset;
            responseMap.response = this.formatResponse(responseMap);
