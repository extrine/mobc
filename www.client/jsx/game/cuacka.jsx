var GameCuacka = React.createClass({
	mixins: [React.addons.PureRenderMixin],

	getInitialState: function () {
		window.swf = this
		this.url = 'swf/cuacka/game' + (this.muted() ? '-muted' : '') + '.swf?23'
		u.download(
			this.url,
			function (percent) {
				if (percent == 100) {
	...
	render: function () {
		return (
			<div className="game-swf">
				...
				{this.state.loaded === true && this.props.room.started ? (
					<div className="game-swf-object">
						<GameCuackaScores room={this.props.room} data={this.props.data} />
						<SWFObject name={this.name()} url={this.url} /> {/*color="#000000"*/}
					</div>
				) : null}
			</div>
		)
