var Game9Ball = React.createClass({
	...
	url: function () {
		return 'swf/pool/game-local.swf?' + this.version
	},
	seed: function () {
		return window.room.round.seed
	},
	background_image: function () {
		return 'https://omgmobc.com/img/background/games-nineballpool-readybg.jpg?3'
	},
	players: function () {
		var players = []
		...
		for (var id in users) {
			if (users[id].game_status == GAME_STATUS_PLAYING) {
				...
				if (window.user.pool && window.user.pool.own) {
					if (!o.pool) o.pool = {}
					o.pool.decal = window.user.pool.decal || ''
					o.pool.table = window.user.pool.table || ''
					o.pool.felt = window.user.pool.felt || ''
				}
				players.push(o)
			}
		}
